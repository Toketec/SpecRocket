...
按「规格编写原则 A」：人为监督划分 + 业务隔离 + 解耦 + token 利用率，不刻意多拆。前端/后端/不同服务天然是拆分边界；AI 起草时只提议拆分方案，定稿权在人。
...
> **不区分项目是否 SpecRocket 基础**。执行 `./spec-rocket migrate` 时：脚本介入扫描结构层（①），AI 先理解项目现状与设计意图（②），保持原有设计不变（③），再补齐骨架、迁移旧结构、收敛游离文档到最新模板标准（④⑤）。
>
> 内容按以下优先级处理：
...
**正确做法**: 按「规格编写原则 A」——有独立边界（前端/后端/不同服务/不同领域）才拆；拆了要能独立交给一个 AI 对话完整实现。
