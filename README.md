...
| 3 | **架构决策无记录** | adrs/ 目录永久留存，新人新 AI 3 分钟看懂全局 |
...
| 🧠 **架构决策** | adrs/ 目录：3 分钟看懂为什么这么设计 | 问 AI："你当初为什么选这个数据库？" |
...
| `migrate` | 嵌入骨架 / 旧版项目升级到最新模板结构（转移 → 收敛 → 保留 → 删除，最终零残留） | 🔄 不碰代码 | 🤖 AI 斜杠命令 |
...
├────────────────────────────────────────────────────────────────────┤
│ Step 2 │ Dev+AI 独作                                                │
│         │ adrs/ + sprints/*/specs/（多 spec 共同促成本次迭代）        │
