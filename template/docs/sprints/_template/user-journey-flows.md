# SPRINT-{NNN}: {冲刺名称} — 用户旅程与流程图

> **负责人**: PM / 产品经理
> **用途**: ① 用户旅程总览——一眼看清本迭代的业务闭环；② 核心流程的各类标准图表（业务流程图 / 时序图 / 状态图）。
> **图表规范**: 优先使用 **Mermaid**（GitHub 原生渲染）；若工具环境不渲染 Mermaid，可用 ASCII 图替代并标注「ASCII 备选」。
> **最少必要原则**: 旅程总览必写；其余图表按业务复杂度按需绘制——能用一个图说清就不画第二个。

---

## 1. 用户旅程总览（必写）

> 一行一格，从用户视角纵览完整闭环：入口 → 关键动作 → 出口。

| 阶段 | 用户目标 | 触点/动作 | 系统响应 | 情绪/痛点 |
|:----|:--------|:--------|:--------|:--------|
| {阶段 1} | {用户想达成什么} | {点击/输入/扫码…} | {系统做什么} | 😀 {顺畅点} / 😠 {痛点} |
| {阶段 2} | {…} | {…} | {…} | {…} |
| {阶段 N} | {…} | {…} | {…} | {…} |

> **验收标准**: 把本表格从上到下读一遍，应能完整复述「用户从哪来、经过什么、到哪去」的业务闭环。

---

## 2. 核心业务流程图

> 泳道图（Mermaid `flowchart` + `subgraph`）：横向/纵向泳道区分参与方（用户 / 前端 / 后端 / 支付…）。

```mermaid
flowchart TD
    subgraph 用户
        A[进入页面] --> B[选择商品]
        B --> C[点击支付]
    end
    subgraph 系统
        C --> D{库存校验}
        D -- 通过 --> E[创建订单]
        D -- 失败 --> F[提示库存不足]
        E --> G[发起支付]
    end
    subgraph 支付渠道
        G --> H[收银台]
        H -- 成功 --> I[支付回调]
        H -- 取消 --> J[订单取消]
    end
    I --> K[完成闭环]
```

> 分支/异常流也要画出来（校验失败、超时、取消），异常分支是业务闭环的一部分。

---

## 3. 关键交互时序图（按需）

> 场景：跨多方的时序交互（如 用户-前端-后端-第三方），用 Mermaid `sequenceDiagram`。

```mermaid
sequenceDiagram
    participant U as 用户
    participant F as 前端
    participant B as 后端
    participant P as 支付渠道

    U->>F: 提交订单
    F->>B: POST /api/orders
    B->>B: 校验+落库
    B-->>F: 201 订单号
    F->>P: 拉起收银台
    P-->>B: 支付回调
    B-->>F: 支付成功
    F-->>U: 展示完成页
```

---

## 4. 状态转换图（按需）

> 场景：业务对象有明确生命周期（订单 / 任务 / 审批流），用 Mermaid `stateDiagram-v2`。

```mermaid
stateDiagram-v2
    [*] --> 待支付
    待支付 --> 已支付: 支付成功
    待支付 --> 已取消: 超时/用户取消
    已支付 --> 已发货
    已发货 --> 已完成: 确认收货
    已完成 --> [*]
```

---

## 5. 图表选择速查

| 想表达什么 | 用什么 | 必写度 |
|:--------|:------|:-----:|
| 业务闭环全貌 | 旅程总览表（§1） | ⭐⭐⭐ 必写 |
| 多方参与的流程走向 | 泳道流程图（§2） | ⭐⭐⭐ 核心流程必写 |
| 跨系统时序交互 | 时序图（§3） | ⭐⭐ 有第三方/多系统时写 |
| 对象生命周期 | 状态图（§4） | ⭐⭐ 有状态机时写 |
| 用例范围/ER 数据模型/其他 | 见 `uml-pack.md` | 按需 |
