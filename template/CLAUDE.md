...
> **规格编写原则**: ①**人为规格划分**——拆几个/边界由 Dev 主导，你只提议不擅自定稿；②**业务隔离**——每 spec 一项独立任务、尽量零耦合，改错不牵连已完成 spec；③解耦+token 利用率上限、不刻意多拆为下限；④**数据/中间件前置**——编码前备好 SQL、数据模型落库、DB、存储、队列；⑤**前端 app 优先**——先做可交互前端，mock 挡 API，后接真实数据。总纲：spec 本质是设计完成后的任务划分、受人监督，你是主要执笔 → 按此收敛方向，禁止自由发挥。
